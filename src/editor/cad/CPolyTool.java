...
public class CPolyTool implements AbstractTool {
	DPoint alt;
	CPoly poly = null;
...
//			GreifTool.item = null;
	...
	public void mousePressed(Matrix m, MouseEvent ev) {
		...
		if (poly != null) {
			...
			poly = new CPoly(alt, new DPoint(x,y),false);
...
//		if (poly != null) {
//			PrivatePoint p = poly.polygon.get(poly.polygon.size() - 1);
//			p.setX(m.getX(ev) - poly.getX());
//			p.setY(m.getY(ev) - poly.getY());
//		}
		greif.draw(g, g2, m);
		if (alt == null) {
			return;
