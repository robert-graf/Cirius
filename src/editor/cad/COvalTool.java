...
public class COvalTool implements AbstractTool {
	...
	public void mousePressed(Matrix m, MouseEvent ev) {
		if (p2 == null) {
			...
			Point a = p1;
			Point b = p2;
			int r1 = (Math.abs(a.x - b.x)) / 2;
			int r2 = (Math.abs(a.y - b.y)) / 2;
			m.addCObject(new COval((a.x < b.x ? a.x : b.x) + r1,
					(a.y < b.y ? a.y : b.y) + r2, r1, r2));
			p2 = null;
			set = p1;
			m.setMainTool(null);
		}
	...
	public void draw(Graphics2D g, Graphics2D g2, Matrix m, MouseEvent ev) {
		...
		Ellipse2D r = new Ellipse2D.Float(a.x < b.x ? a.x : b.x,
				a.y < b.y ? a.y : b.y, Math.abs(a.x - b.x) - 1, Math.abs(a.y
						- b.y) - 1);
